...
SAS
In SAS, add the option 'guessingrows' to the import statement. The 2025 Sample Adult file has 24,215 
observations and the example below specifies to use 5,000 observations to determine the length:

proc import datafile="adult25.csv" out=adult dbms=csv; getnames=yes; guessingrows=5000; run;
...
the entire 2025 Sample Adult CSV file and does not convert text strings into factors but leaves them as 
text strings. 

adult2025 <- read.csv("adult25.csv", stringsAsFactors=FALSE)


Library(readr)
The read_csv function by default only reads in 1,000 records to determine variable attributes and will not 
convert strings to factors. If using read_csv, add the argument 'guess_max.' The 2025 Sample Adult file has 
24,215 observations and the example below specifies to use 24,215 observations to determine the data type.

adult2025 <- read_csv("adult25.csv", guess_max = 24215)
